...
RUN pip3 install -i https://pypi.tuna.tsinghua.edu.cn/simple onnxruntime simple_pid
...
CMD source /opt/workspace/devel_isolated/setup.bash && catkin_make install --use-ninja -DSETUPTOOLS_DEB_LAYOUT=OFF; chmod +x /opt/ep_ws/src/rmus_solution/scripts/*; /opt/ros/noetic/env.sh /opt/ep_ws/devel/env.sh /opt/start.sh
