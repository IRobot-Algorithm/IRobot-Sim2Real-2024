...
def grip(gameinfo , is_here, response):
    for i, target in enumerate(gameinfo.data):
        ...
        if target in blockinfo.data and is_here == 0:#判断是否识别到了目标方块
            ...
            try:
                ...
                if target in blockinfo.data :
                    times = 0#尝试重新抓取的次数
                    while target in blockinfo.data and times<2:#若抓不上会尝试抓20次
                        times = times + 1
...
if __name__ == '__main__':
    ...
    for i in range(0,3):
        ...
        if is_here == 2:
            #去此矿区的另一侧
            is_here, response = go_to_another_side(location, gameinfo, is_here, response)
